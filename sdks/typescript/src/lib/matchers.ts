 ...
 * @generated-id: 2597cf31bd6a
 */

import { MailmonDefaultError } from "../models/errors/mailmon-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | MailmonDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | MailmonDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new MailmonDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new MailmonDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
