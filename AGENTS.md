...
- async transport adapters: `packages/queue/src/index.ts`
...
- Gmail refresh tokens are now encrypted at rest before writes to `gmail_mailbox_credentials`; API and worker runtimes require `MAILMON_GMAIL_REFRESH_TOKEN_ENCRYPTION_KEY`, and the CLI needs the same key for local sync flows that read persisted Gmail credentials.
- Revoked or unrefreshable Gmail refresh tokens, plus unreadable or missing persisted Gmail credentials, now move mailboxes into `reconnect_required`; sync runs record that outcome without retrying transport-level dispatch for mailboxes already awaiting reconnect.
